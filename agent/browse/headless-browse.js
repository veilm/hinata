(function () {
	// If the script is already loaded, skip re-initialization to avoid errors and overwrites.
	if (window.llmPack) {
		return;
	}
			...
			for (const prop of node.meaningfulReason) {
				...
				if (
					...
					if (displayValue.length > effectiveCropLength) {
						displayValue =
							displayValue.substring(0, effectiveCropLength) + "...";
					}
				}
	...
	function llmDisplay() {
		...
		if (window.contentTree) {
			...
			// console.log(JSON.stringify(window.contentTree, null, 2));

			// Format the tree to a string

			// Display the formatted tree in an overlay
			if (window.formattedTree) {
				displayTreeOverlay(window.formattedTree);
			} else {
				console.log(
	...
	function llmDisplayVisual(showCloseButton = false) {
		...
		if (!window.contentTree) {
			console.log(
				"No content tree available to visualize. Run llmPack() first.",
			);
			return;
		}
	...
	window.llmPack = llmPack;
	window.llmDisplay = llmDisplay;
	window.llmDisplayVisual = llmDisplayVisual;
})();
