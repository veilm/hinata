...
def execute_command_and_get_output(master_fd, command_str):
    ...
    if "\n" in command_str:
        raise ValueError(
            "Command string must not contain newlines for this simplified parser."
        )
    ...
    while True:
        if time.time() - start_time > timeout_seconds:
            error_message = (
                f"Timeout waiting for marker for command: '{command_str}'.\n"
            )
    ...
    # Adjust end_idx to remove the echoed marker command and any subsequent blank lines.
    # Iterate backwards from the end of candidate_lines.
    end_idx = len(candidate_lines)

    # Search backwards from the end of candidate_lines (before the marker_line itself)
    # to find the line containing the echoed marker command.
    # Everything from that echoed command line onwards (within candidate_lines) is noise.
    for i in range(len(candidate_lines) - 1, start_idx - 1, -1):
        line_content = candidate_lines[i].strip()
        if f"echo '{COMMAND_EXECUTION_MARKER}'" in line_content:
            # Found the echoed marker command. The actual output ends before this line.
            end_idx = i
            break
        elif line_content == "":
            # This is a blank line. It might be between actual output and marker echo,
            # or after marker echo. We effectively trim it by potentially setting end_idx lower.
            continue
        else:
            # This is a non-blank line that isn't the marker echo.
            # This means this line IS part of the command's actual output.
            # So, everything up to and including this line is output.
            # The loop breaks, and end_idx remains len(candidate_lines) or a value
            # set by a previous iteration that found a blank line.
            # More accurately, end_idx should be i + 1 to include this content line.
            # However, because we initialize end_idx = len(candidate_lines) and only reduce it
            # if we find the marker_echo or pass over blank lines before marker_echo,
            # breaking here correctly preserves all content up to this point.
            break

    actual_output_lines = []
    if start_idx < end_idx:  # Ensure slice is valid and non-empty
        actual_output_lines = candidate_lines[start_idx:end_idx]
    elif start_idx == end_idx:  # Slice would be empty
        actual_output_lines = []
    # If start_idx > end_idx, this implies an issue, treat as no output.
    # This case should ideally not happen with correct start_idx and end_idx logic.
    # For robustness, ensure actual_output_lines is at least empty list.
    else:
        actual_output_lines = []
...
def pty_demo():
    ...
    try:
        ...
        # Set PS1 to empty string to avoid prompt interference
        print("Setting PS1='' in the PTY shell...")
        # The output of PS1="" itself is not important, but we run it through the helper
        # to ensure it completes before other commands.
        ps1_output = execute_command_and_get_output(master_fd, 'PS1=""')
        if ps1_output:  # Should ideally be empty or just the command echo
            print(f"Output from setting PS1 (should be minimal): '{ps1_output}'")
        print("PS1 set.")
        ...
        print(
            f"Running 'cd $HOME' (shell will expand $HOME, Python expects: '{expected_home_dir}')..."
        )
        ...
        print(
            f"Expected PWD after 'cd $HOME' (from os.path.expanduser): '{expected_home_dir}'"
        )
        ...
        if final_pwd == expected_home_dir:
            ...
            print(
                f"FAILURE: PWD did not change to $HOME correctly. Got '{final_pwd}', expected '{expected_home_dir}'."
            )
            if initial_pwd == final_pwd and initial_pwd != expected_home_dir:
                print(
                    "Note: PWD did not change at all. 'cd $HOME' might have failed or led to the same directory."
                )
            elif initial_pwd == expected_home_dir and final_pwd == expected_home_dir:
                print(
                    "Note: Initial PWD was already $HOME. 'cd $HOME' correctly resulted in PWD being $HOME."
                )
