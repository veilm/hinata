...
// streamLLMResponseV2 uses the new OutputCoordinator for better synchronization
func (a *Agent) streamLLMResponseV2(config llm.Config, packedContent string) (string, string, error) {
	ctx := context.Background()
	eventChan, errChan := llm.StreamLLMResponse(ctx, config, packedContent)
	...
	// Setup streaming spinner using V2 with OutputCoordinator
	sp := a.getSelectedSpinner()
	colorFunc := func(s string) {
		a.theme.Spinner.Print(s)
	}
	streamSpinner := streamspinner.NewV2(sp, marginStr(), colorFunc)
	...
	for {
		...
		case event, ok := <-eventChan:
			if !ok {
				// Channel closed - flush any remaining content
				if line := contentBuffer.Flush(); line != nil {
					a.printBufferedLineV2(*line, streamSpinner, tagParser, a.theme.DefaultText)
				}
				if line := reasoningLineBuffer.Flush(); line != nil && !a.IgnoreReasoning {
					a.printBufferedLineV2(*line, streamSpinner, nil, a.theme.Reasoning)
				}
...
// printBufferedLineV2 prints a complete line using the V2 spinner
func (a *Agent) printBufferedLineV2(line string, spinner *streamspinner.StreamingSpinnerV2, tagParser *TagParser, defaultColor *color.Color) {
	if tagParser != nil {
		// Parse for shell blocks and apply appropriate coloring
		results := tagParser.Parse(line)
